...
ImageProcessor::ImageProcessor() : mean_scaling( 0.485f, 0.456f, 0.406f ), std_scaling( 0.229f, 0.224f, 0.225f )
{
...
void ImageProcessor::normalize_and_transpose( cv::Mat &image, float *dest, int dim_x, int dim_y )
{
    ...
    for ( int channel = 0; channel < 3; channel++ )
    {
        ...
        for ( int i = 0; i < stride; i++ )
        {
            ...
            from_reference /=
                std_scaline_for_channel; /* remove internal for for loop of cv::divide(image, std_scaling, image); */
            from_reference -= mean_scaling_for_channel; /* remove internal for for loop of cv::subtract(image,
                                                           mean_scaling, image); */
