...
UpdateChecker::UpdateChecker( std::string &version, IUpdateSub *obs ) : current_version( version )
{
    this->request  = QNetworkRequest();
    this->observer = obs;
}

UpdateChecker::UpdateChecker( const std::string &&version, IUpdateSub *obs ) : current_version( version )
{
    this->request  = QNetworkRequest();
    this->observer = obs;
}
...
{
    QObject::connect( &manager, SIGNAL( finished( QNetworkReply * ) ), this, SLOT( callback( QNetworkReply * ) ) );
